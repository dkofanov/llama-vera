...
struct sampler_ctx {
    ...
    sampler_ctx(const llama_vocab * v, vera_mode mode) : vocab(v), checker(mode) {}
};
...
static const char * sampler_name(const struct llama_sampler *) {
    return "llama-vera-sampler";
}

static void rebuild_mask(sampler_ctx * ctx) {
...
static void sampler_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    sampler_ctx * ctx = static_cast<sampler_ctx *>(smpl->ctx);
    if (!ctx->mask_valid) {
        rebuild_mask(ctx);
    }
...
static void sampler_accept(struct llama_sampler * smpl, llama_token token) {
    sampler_ctx * ctx = static_cast<sampler_ctx *>(smpl->ctx);
...
static void sampler_reset(struct llama_sampler * smpl) {
    sampler_ctx * ctx = static_cast<sampler_ctx *>(smpl->ctx);
    ctx->checker.reset();
    ctx->has_candidates = true;
    ctx->mask_valid = false;
...
static struct llama_sampler * sampler_clone(const struct llama_sampler * smpl) {
    const sampler_ctx * src = static_cast<const sampler_ctx *>(smpl->ctx);
    sampler_ctx * dst = new sampler_ctx(src->vocab, src->checker.mode());
...
static void sampler_free(struct llama_sampler * smpl) {
    delete static_cast<sampler_ctx *>(smpl->ctx);
}

static struct llama_sampler_i sampler_iface = {
    sampler_name,
    sampler_accept,
    sampler_apply,
    sampler_reset,
...
struct llama_sampler * llama_vera_sampler_create(const struct llama_vocab * vocab, vera_mode mode) {
    return llama_sampler_init(&sampler_iface, new sampler_ctx(vocab, mode));
}
...
bool llama_vera_sampler_has_candidates(const struct llama_sampler * smpl) {
    return static_cast<const sampler_ctx *>(smpl->ctx)->has_candidates;
}
